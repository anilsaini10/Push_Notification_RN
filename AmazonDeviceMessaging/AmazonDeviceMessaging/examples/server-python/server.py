...
PORT = INSERT_SERVER_PORT_HERE

# Client ID received from ADM.
PROD_CLIENT_ID = "INSERT_CLIENT_ID_HERE"

# Client secret received from ADM.
PROD_CLIENT_SECRET = "INSERT_CLIENT_SECRET_HERE"
...
class SampleADMWebapp:
    ...
    def send_message_to_device(self, message, device, consolidationKey, expiresAfter):
        ...
        JSON_MSG_REQUEST['md5'] = self.calculate_checksum(JSON_MSG_REQUEST['data']).decode('utf-8')
        print(req.headers)
        print(req.get_full_url())
        print(JSON_MSG_REQUEST)
        try:
            # POST EnqueueMessage request to AMD Servers.
            response = urllib.request.urlopen(req,json.dumps(JSON_MSG_REQUEST).encode('utf-8'))
...
#
        ...
        for key in message_keys:
            message_string = message_string + key
            message_string = message_string + ':'
            message_string = message_string + data[key]
            if key != message_keys[-1]:
                message_string = message_string + ','

        # Converting data to UTF-8 and computing MD5 as per RFC 1321.
        md5 = hashlib.md5(message_string.encode('utf-8')).digest()

        # Base 64 encoding.
        md5_checksum = base64.standard_b64encode(md5)

        print("App server md5_checksum " + str(md5_checksum))
        return md5_checksum
    ...
    def request_token(self):
        ...
        req = urllib.request.Request(AMAZON_TOKEN_URL)
        req_data = urllib.parse.urlencode(ACCESS_TOKEN_REQUEST_DATA)
        print(req_data, str(len(req_data)))
        req.add_header('Content-Type', 'application/x-www-form-urlencoded')
        
        # Converting data to UTF-8 for POST
